...
async function main() {
  ...
  // Official tracker (Google - CFT target/achievement)
  const off = await getOfficialTracker();
  const g = off.glance;

  // iBOS ERP (DWH) - all panels
  const [orders, inv, del, coll, byPlant, gradeMix, force, cov, ob, daily, rev, target, active] = await Promise.all([
    ...
    db.request().input('u', sql.Int, UNIT).input('f', sql.Date, F).input('t', sql.Date, T).query(`SELECT strSubGLName name, SUM(numAmount) v FROM fin.tblAccountingJournalArc WHERE intBusinessUnitId=@u AND isActive=1 AND dteTransactionDate>=@f AND dteTransactionDate<=@t AND strGeneralLedgerName='Sales (Local)' GROUP BY strSubGLName`),
    db.request().query(`SELECT numTarget FROM pms.tblTargetSetupArc WHERE intBusinessUnitId=175 AND isActive=1 AND numTarget>0 AND strFrequencyValue='Aug, 2026' AND intKPIsId=2401`),
    db.request().input('u', sql.Int, UNIT).input('f', sql.Date, F).input('t', sql.Date, T).query(`SELECT COUNT(DISTINCT intSoldToPartnerId) n FROM oms.tblSalesOrderHeaderArc WHERE intBusinessUnitId=@u AND isActive=1 AND dteSalesOrderDate>=@f AND dteSalesOrderDate<=@t`)
  ]);
  ...
  const activeCust = num(active.recordset[0].n);
  ...
  const revTarget = num(target.recordset[0] ? target.recordset[0].numTarget : 0);

  const data = {
    meta: {
      title: 'ARMCL Live SMS Control Tower — iBOS ERP',
      company: 'Akij Ready Mix Concrete Ltd (ARMCL)',
      source: 'iBOS ERP archive (DWH) · ARMCL = BU 175 · ' + new Date().toISOString(),
    ...
    kpi: {
      revenueTargetBDT: round(revTarget, 0),
      revenueBDT: round(revenue, 0),
      salesOrders: ordCnt,
      ...
      universe,
      coveragePct: round(universe > 0 ? activeCust / universe * 100 : 0, 1),
  ...
  fs.writeFileSync(path.join(__dirname, 'sms-control-tower', 'ibos-live-data.json'), JSON.stringify(data, null, 2), 'utf8');
  console.log('Written: sms-control-tower/ibos-live-data.json');
  console.log('Revenue target:', revTarget, '| Revenue (Sales Local):', revenue);
  console.log('Orders:', ordCnt, '| Order value:', ordVal, '| Active customers:', activeCust);
  console.log('Collection:', collected, '| Sales force:', force.recordset.length, '| Plants:', byPlant.recordset.length);
  await db.close();
}
