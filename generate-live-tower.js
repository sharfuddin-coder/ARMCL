...
// iBOS Setup Base Achievement captured data (person/team/customer-wise)
...
<title>ARMCL Live SMS Control Tower | iBOS ERP + Google Tracker</title>
<style>
...
<div class="topbar">
  <div>
    <h1>ARMCL Live SMS Control Tower</h1>
...
<footer>Sources: Official Google Tracker (CFT, live) + iBOS ERP Setup Base Achievement (person-wise) · ARMCL = BU 175 · Framework: Akij Resource — Sales Management OS</footer>

<script>
// ===== iBOS ERP Setup Base Achievement (captured offline, embedded) =====
var SBA = ${JSON.stringify(sba)};

var SPREADSHEET_ID = '1vPlcijsZkj4p6ZHmzg7jEAutNrW5l2YKlbNUtgXkNbI';
var GID_TARGET = '990537426';
var GID_DAYWISE = '2096727161';
...
function loadSheet(gid, cb){
  ...
  window[cbName] = function(resp){
    ...
    var s=document.getElementById('gs_'+gid); if(s) s.remove();
    cb(resp.table);
  };
...
function parseTarget(t){
  ...
  d.rows.forEach(function(row,i){
    if(i<3)return;
    var name=String(row[1]||'').trim(), dg=String(row[2]||'').trim();
    var gk=String(row[15]||'').trim();
    if(gk){var gv=String(row[16]||'').trim(); if(gv)gl[gk]=gv;}
    if(name&&dg&&/^\\d+$/.test(String(row[0]).trim())){
      emp.push({name:name,dg:dg,target:cleanNum(row[3]),sales:cleanNum(row[5]),ach:cleanNum(row[7])});
    }
  });
  return {emp:emp,gl:gl};
}
function parseDaywise(t){
  var d=tableToArrays(t), header=d.rows[1]||[], armcl=null;
  for(var i=0;i<d.rows.length;i++) if(String(d.rows[i][0]).indexOf('ARMCL')>=0) armcl=d.rows[i];
  if(!armcl)return{days:[]};
  var days=[], now=new Date(), dayNum=0;
  for(var c=2;c<=23&&c<armcl.length;c++){
    dayNum++;
    var h=String(header[c]||'').trim(), m=h.match(/^(\\d{2})-(\\d{2})-\\d{4}$/);
    var dd=m?m[1]:('0'+dayNum).slice(-2), mm=m?m[2]:('0'+(now.getMonth()+1)).slice(-2);
    days.push({date:now.getFullYear()+'-'+mm+'-'+dd,qty:cleanNum(armcl[c])});
  }
  return days;
}

function render(gl, days, dayTotal){
  var root=document.getElementById('root');
  root.innerHTML='';
  var s=SBA.totals;
  document.getElementById('subtitle').textContent = 'Akij Ready Mix Concrete Ltd (ARMCL) · Aug 2026 MTD · Official tracker + iBOS Setup Base Achievement';

  var ach = cleanNum(gl['Achiv % till date']);
  var target = cleanNum(gl['Monthly Target']) || s.targetQty || 1230000;
  var sales = cleanNum(gl['Sales Till Date']);
  ...
  var cards=[
    ...
    {l:'Active Customers',v:fmt(SBA.topCustomers.length),c:'blue'},
    {l:'Sales Persons',v:fmt(SBA.byPerson.length),c:'blue'},
    {l:'Teams',v:fmt(SBA.byTeam.length),c:'blue'},
    {l:'Day-wise Days',v:days.length,c:'blue'}
  ];
  ...
  gs.innerHTML=[
    ['SALES', rag(ach,95,80)],
    ['CASH', rag(s.collectionVsSalesPct,80,60)],
    ['CREDIT', rag(s.collectionVsSalesPct,80,60)],
    ['SERVICE','amber'],['COVERAGE','amber'],['MARGIN','amber'],['STOCK','amber'],['DATA','blue']
  ].map(function(x){return '<div class="g"><span>'+x[0]+'</span><span class="dot '+x[1]+'"></span></div>';}).join('');
  root.appendChild(gs);

  // Row1: daily trend + person-wise
  ...
  var maxD=1; days.forEach(function(x){if(x.qty>maxD)maxD=x.qty;});
  bDaily.innerHTML='<h3>Daily Sales Trend (CFT, MTD)</h3><div class="daily">'+
    days.map(function(x){var h=Math.max(2,x.qty/maxD*100);
    ...
    '</div><div class="note">MTD: '+fmt(dayTotal)+' CFT / Target '+fmt(target)+' CFT · Live from Google Sheet (gid '+GID_DAYWISE+')</div>';

  var bPerson=document.createElement('div'); bPerson.className='box'; r1.appendChild(bPerson);
  var maxP=1; SBA.byPerson.forEach(function(x){if(x.qty>maxP)maxP=x.qty;});
  bPerson.innerHTML='<h3>Sales Person Wise — Qty (iBOS Setup Base Achievement)</h3>'+
    SBA.byPerson.slice().sort(function(a,b){return b.qty-a.qty;}).map(function(x){
      return bar(esc(x.name),x.qty,maxP,'blue',fmt(x.qty)+' CFT · '+pct(x.achivPct));
    }).join('')+
    '<div class="note">Source: iBOS ERP report setupbaseachivement (Aug MTD). Click Refresh to reload.</div>';

  // Row2: team-wise + top customers
  ...
  var bTop=document.createElement('div'); bTop.className='box'; r2.appendChild(bTop);
  bTop.innerHTML='<h3>Top 10 Customers (by Sales)</h3><div style="overflow-x:auto"><table><thead><tr><th>Customer</th><th>Qty</th><th>Sales</th><th>Collection</th><th>Ach %</th></tr></thead><tbody>'+
    SBA.topCustomers.slice(0,10).map(function(c){
      return '<tr><td>'+esc(c.customer)+'</td><td class="num">'+fmt(c.monthlyQty)+'</td><td class="num">'+fmt(c.monthlySales)+'</td><td class="num">'+fmt(c.collection)+'</td><td class="num">'+pct(c.achvPct)+'</td></tr>';
    }).join('')+'</tbody></table></div>';

  // Row3: employee tracker (official) + collection summary
  var r3=document.createElement('div'); r3.className='row'; root.appendChild(r3);
  var bEmp=document.createElement('div'); bEmp.className='box'; r3.appendChild(bEmp);
  var emps = window.__targetEmp || [];
  bEmp.innerHTML='<h3>Official Sales Force Tracker</h3>'+(emps.length?
    '<div style="overflow-x:auto"><table><thead><tr><th>Employee</th><th>Designation</th><th>Target</th><th>Sales</th><th>Ach %</th></tr></thead><tbody>'+
    emps.sort(function(a,b){return b.sales-a.sales;}).map(function(e){
    ...
    }).join('')+'</tbody></table></div>':'<div class="note">Loading from Google Sheet…</div>');
...
function load(){
  ...
  loadSheet(GID_TARGET, function(t1){
    ...
    loadSheet(GID_DAYWISE, function(t2){
      var days = parseDaywise(t2);
      var total = days.reduce(function(s,x){return s+x.qty;},0);
      render(p.gl, days, total);
    });
  });
}
load();
...
`;
fs.writeFileSync(path.join(__dirname, 'sms-control-tower', 'live-tower.html'), html, 'utf8');
console.log('Live tower written: sms-control-tower/live-tower.html |', html.length, 'bytes');
